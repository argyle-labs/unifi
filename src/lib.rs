//! unifi service backend — UniFi network controller.
//!
//! Implements `ServiceBackend` so the generic `service.*` tools
//! (deploy/backup/restore/configure/status/connect/sync) drive unifi. No
...
/// unifi backend. Holds only the provider name; per-instance endpoint/creds
/// come from the `Endpoint` the generic `service.*` tools hand each op.
#[derive(Debug, Clone)]
pub struct UnifiBackend {
    provider: &'static str,
}
...
impl ServiceBackend for UnifiBackend {
    fn provider(&self) -> &str { self.provider }

    /// Runtimes unifi can be placed on. `service.deploy` hands the
        ...
        -> BoxFuture<'a, Result<WorkloadSpec, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("unifi.workload_spec")) })
    }
        ...
        -> BoxFuture<'a, Result<BackupArtifact, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("unifi.backup")) })
    }

    fn restore<'a>(&'a self, _ep: &'a Endpoint, _from: &'a BackupArtifact)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        Box::pin(async move { Err(ServiceError::unimplemented("unifi.restore")) })
    }

    fn configure<'a>(&'a self, _ep: &'a Endpoint, _config: &'a str)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        // TODO: apply unifi-specific config idempotently.
        Box::pin(async move { Err(ServiceError::unimplemented("unifi.configure")) })
    }
        ...
        -> BoxFuture<'a, Result<ServiceStatus, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("unifi.status")) })
    }
}
...
mod tests {
    ...
    fn declares_provider() {
        let b = UnifiBackend::new("unifi");
        assert_eq!(b.provider(), "unifi");
    }
}
